...
var serviceUrl =
  'http://172.20.83.218:8090/iserver/services/transportationAnalyst-Network-2/rest/networkanalyst/MapManager_Network@MapManager'
...
const findPath = () => {
  ...
  if (!startPt) {
    window.$message.info('请选择火灾点！')
  }
  if (!endPt) {
    window.$message.info('请选择救援队伍！')
  }
  const [startX, startY] = startPt.getGeometry().getCoordinates()
  const [endX, endY] = endPt.getGeometry().getCoordinates()
  nodeArray = [new SuperMap.Geometry.Point(startX, startY), new SuperMap.Geometry.Point(endX, endY)]
  ...
  // 进行查找
  new NetworkAnalystService(serviceUrl).findPath(
    findPathParameter,
    function(serviceResult) {
      ...
      serviceResult.result.pathList.map(function(result) {
        ...
        window.$map.addLayer(lineLayers)
        // 添加分析出的引导点
