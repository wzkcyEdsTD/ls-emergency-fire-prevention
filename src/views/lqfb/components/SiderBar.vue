...
export default {
  ...
  methods: {
    ...
    searchFeatures(node) {
      return new Promise((resolve) => {
        const sqlParam = new SuperMap.GetFeaturesBySQLParameters({
          ...
          datasetNames: [`lishui_forestfire_v2:${node.dataset}`],
        });
        const url =
    ...
    async handleCheckChange(data, checked, id) {
      ...
      if (data.label === "小流域监控") {
        if (!this.xlyVideoLayer) {
          const serviceResult = await this.searchFeatures(data);
          const videoPointList = serviceResult.result.features.features;
          const features = [];
          that.xlyVideoList = videoPointList;
          const style = new Style({
            image: new Icon({
              anchor: [0.5, 26],
              anchorXUnits: "fraction",
          ...
          this.xlyVideoLayer = new VectorLayer({
            source: vectorSource,
          });
          this.$map.addLayer(this.xlyVideoLayer);
          that.$bus.$emit("showXLYVideoList", true);
          that.$bus.$emit("sendXLYVideoListData", that.xlyVideoList);
          this.xlyVideoTemp = true;
          this.$bus.$emit("allLegend", {
            temp: this.xlyVideoTemp,
            label: data.label,
          });
        } else {
          if (this.xlyVideoTemp) {
            this.xlyVideoTemp = false;
            that.$bus.$emit("showXLYVideoList", false);
            this.$bus.$emit("clearVideoMaker", true);
            this.$bus.$emit("allLegend", {
              temp: this.xlyVideoTemp,
              label: data.label,
            });
          } else if (!this.xlyVideoTemp) {
            this.xlyVideoTemp = true;
      ...
      if (data.label === "小学学区") {
        if (!this.primarySchoolDistrictLayer) {
          const url =
            "http://10.53.137.59:8090/iserver/services/map-lishui_forestfire_v2/rest/maps/jy_primaryschool_district@lishui_forestfire_v2";
          this.primarySchoolDistrictLayer = this.$map.createTileSuperMapRestLayer(
            url,
            {
          ...
          this.$bus.$emit("allLegend", {
            temp: this.primarySchoolDistrictTemp,
            label: data.label,
          });
          ...
          this.primarySchoolDistrictLayer.setVisible(
            this.primarySchoolDistrictTemp
          );
        }
      }
      if (data.label === "小学适龄儿童") {
        if (!this.primarySchoolChildrenLayer) {
          ...
          this.$bus.$emit("allLegend", {
            temp: this.primarySchoolChildrenTemp,
            label: data.label,
          });
          ...
          this.primarySchoolChildrenLayer.setVisible(
            this.primarySchoolChildrenTemp
          );
        }
      }
      if (data.label === "初中") {
        if (!this.middleschoolPointLayer) {
          ...
          this.$bus.$emit("allLegend", {
            temp: this.middleschoolPointTemp,
            label: data.label,
          });
      ...
      if (data.label === "初中学区") {
        if (!this.middleSchoolDistrictLayer) {
          const url =
            "http://10.53.137.59:8090/iserver/services/map-lishui_forestfire_v2/rest/maps/jy_middleschool_district@lishui_forestfire_v2";
          this.middleSchoolDistrictLayer = this.$map.createTileSuperMapRestLayer(
            url,
            {
          ...
          this.$bus.$emit("allLegend", {
            temp: this.middleSchoolDistrictTemp,
            label: data.label,
          });
          ...
          this.middleSchoolDistrictLayer.setVisible(
            this.middleSchoolDistrictTemp
          );
        }
      }
      if (data.label === "初中适龄儿童") {
        if (!this.middleSchoolChildrenLayer) {
          ...
          this.$bus.$emit("allLegend", {
            temp: this.middleSchoolChildrenTemp,
            label: data.label,
          });
          ...
          this.middleSchoolChildrenLayer.setVisible(
            this.middleSchoolChildrenTemp
          );
        }
      }
      if (data.label === "火灾报警点") {
