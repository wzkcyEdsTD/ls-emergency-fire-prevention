    ...
    <div class="ljxq-container">
      <div class="zbjksz-container">
        <div class="head" style="display:flex">
          ...
          <el-select v-model="value" filterable  placeholder="请选择区县" @change="selectOne">
            <el-option
        ...
        <el-collapse v-model="activeNames" accordion>
          <el-collapse-item :title='xlyjk' name="1">
              <div class="ul-head">
                <div class="item item-1">设备名称</div>
                <div class="item item-1">区县</div>
              </div>
            <ul style="max-height:75vh">
              <li v-for="(item, index) in videoList" 
                ...
                <div @mouseenter="titeEnter" class="item item-2">{{ item.properties.DISTRICT }}</div>
              </li>
...
export default {
  data() {
    return {
      // districtList:["景宁县","庆元县","云和县","松阳县","遂昌县","缙云县","青田县","龙泉市","莲都区"],
      options: [{
        value: '全部',
        label: '全部'
      },{
  ...
  watch:{
    searchTextAll(text){
      ...
      if (this.value!='全部') {
        this.videoList = this.videoList.filter(v=>v.properties.DISTRICT.indexOf(this.value)!=-1)
      }
    }
  },
  methods: {
    selectOne(event, item) {
      if (this.searchTextAll) {
        this.videoList = this.ttVideoList.filter(v=>v.properties.MC.indexOf(this.searchTextAll)!=-1)
      }else{
        this.videoList = this.ttVideoList
      }
      if (event !== '全部') {
        this.videoList = this.videoList.filter(v=>v.properties.DISTRICT.indexOf(event)!=-1)
      }
      this.hasMore = false;
    },
    ...
    handleVideoClick(v){
        ...
        that.$parent.$refs.videoList.addGifMarks([v.properties.X,v.properties.Y])
        setTimeout(()=>{
  ...
  mounted() {
    const that = this;
    that.$bus.$on("showSLSPVideoList", (value) => {
      if (value) {
        that.$nextTick(() => {
    ...
    that.$bus.$on("sendSLSPVideoListData", (videoPointList) => {
      that.$nextTick(() => {
        ...
        that.xlyjk = `水利视频监控列表（${that.ttVideoList.length}）`
        that.searchTextAll = "",
        that.value = ""
      });
    });
    ...
    .zbjksz-container {
      .head{
        ...
        .select-class{
          ...
          .option-class >:hover{
            background: url('~@/common/images/底.png') !important;
            background-size: 100% 100%;
          }
        }
      }
