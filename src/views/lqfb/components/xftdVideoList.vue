...
export default {
  ...
  mounted() {
    ...
    that.$bus.$on("showXFTDVideoList", (value) => {
      if (value) {
        that.$nextTick(() => {
    ...
    that.$bus.$on("sendXFTDVideoListData", (videoPointList) => {
      that.$nextTick(() => {
        ...
        that.xlyjk = `消防通道视频监控列表（${that.ttVideoList.length}）`
      });
    });
  },
  beforeDestroy() {
    const that = this;
    that.$bus.$off("showXFTDVideoList");
    that.$bus.$off("sendXFTDVideoListData");
  },
};
</script>
